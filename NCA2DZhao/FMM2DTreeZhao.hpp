...
public:
  ...
  std::vector<int> chargeLocations;
	...
	std::vector<int> incoming_chargePoints_local;//equivalent points {y_{k}^{B,i}}
	std::vector<int> incoming_checkPoints_local;//check points {x_{k}^{B,i}}
...
public:
  ...
  std::vector<pts2D> gridPoints; //all particles in domain
	int TOL_POW;
// public:
	FMM2DTree(kerneltype* K, int nLevels, int nParticlesInLeafAlong1D, double L, int TOL_POW) {
		this->K					=	K;
  ...
  void assignLeafChargeLocations() {
	...
	void getNodes() {
		...
		for (int j=2; j<=nLevels; j++) {
			getNodes_incoming_level_topDown(j);
		}
	}
	...
	void getParticlesFromChildren_incoming(int j, int k, std::vector<int>& searchNodes) {
		if (j==nLevels) {
			...
			int J = j+1;
			for (int c = 0; c < 4; c++) {
				searchNodes.insert(searchNodes.end(), tree[J][4*k+c].incoming_checkPoints_local.begin(), tree[J][4*k+c].incoming_checkPoints_local.end());
			}
		}
	}

	void getParticlesFromChildren_incoming_topDown(int j, int k, std::vector<int>& searchNodes, std::vector<std::pair<int, int> >& desc_clusters) {
		if (j==nLevels) {
			searchNodes.insert(searchNodes.end(), tree[j][k].chargeLocations.begin(), tree[j][k].chargeLocations.end());
		}
		else {
			for (size_t c = 0; c < desc_clusters.size(); c++) {
				int J = desc_clusters[c].first;
				int K = desc_clusters[c].second;
				searchNodes.insert(searchNodes.end(), tree[J][K].incoming_checkPoints_local.begin(), tree[J][K].incoming_checkPoints_local.end());
			}
		}
	}

	int count_pivots(std::vector<std::pair<int, int> >& clusters) {
		int sum = 0;
		for (size_t i = 0; i < clusters.size(); i++) {
			int j = clusters[i].first;
			int k = clusters[i].second;
			sum += tree[j][k].incoming_checkPoints_local.size();
		}
		return sum;
	}

	void getOffSprings(int j, int k, std::vector<std::pair<int, int> >& desc_clusters, int bound){
		if (j==nLevels || tree[j][k].incoming_checkPoints_local.size() > bound) {
			std::pair<int, int> temp;
			temp.first = j;
			temp.second = k;
			desc_clusters.push_back(temp);
			return;
		}
		else {
			...
			pair_temp.first = j;
			pair_temp.second = k;
			desc_clusters.push_back(pair_temp);
			while(count_pivots(desc_clusters) < bound) {
				// std::cout << "count_pivots(desc_clusters): " << count_pivots(desc_clusters) << " bound: " << bound << std::endl;
				std::vector<std::pair<int, int> > temp;
				temp = desc_clusters;
				desc_clusters.clear();
				for (size_t i = 0; i < temp.size(); i++) {
					int ji = temp[i].first;
					int ki = temp[i].second;
					if (ji==nLevels) {
						std::pair<int, int> pair_temp;
						pair_temp.first = ji;
						pair_temp.second = ki;
				...
				if (desc_clusters.size() == temp.size()) {
					break;
				}
				temp.clear();
			}
	...
	void getNodes_incoming_box_topDown(int j, int k, int& ComputedRank) {
		std::vector<int> IL_Nodes;//indices
		IL_Nodes.insert(IL_Nodes.end(), tree[j][k].incoming_chargePoints_local.begin(), tree[j][k].incoming_chargePoints_local.end());
		if (j>2) {
			IL_Nodes.insert(IL_Nodes.end(), tree[j-1][k/4].incoming_chargePoints_local.begin(), tree[j-1][k/4].incoming_chargePoints_local.end());
		}
		// std::cout << "IL_Nodes.size(): " << IL_Nodes.size() << std::endl;

		std::vector<int> boxA_Nodes;
		std::vector<std::pair<int, int> > desc_clusters;
		getOffSprings(j, k, desc_clusters, IL_Nodes.size());
		// std::cout << "j: " << j << " k: " << k << "	DC: " << desc_clusters.size() << std::endl;
		for (size_t i = 0; i < desc_clusters.size(); i++) {
			int ji = desc_clusters[i].first;
			int ki = desc_clusters[i].second;
			// std::cout << "(" << ji << "," << ki << ")" << ", ";
		}
		// std::cout << std::endl;
		// std::cout << "here" << std::endl;
		getParticlesFromChildren_incoming_topDown(j, k, boxA_Nodes, desc_clusters);
		// std::cout << "here" << std::endl;

		std::vector<int> row_bases, col_bases;
		Eigen::MatrixXd Ac, Ar;
		if (boxA_Nodes.size() <= IL_Nodes.size()) {
			...
			Eigen::MatrixXd L_trans, R_trans;
			LowRank* LR		=	new LowRank(K, TOL_POW, IL_Nodes, boxA_Nodes);
			LR->ACA_only_nodes(row_bases, col_bases, ComputedRank, Ac, Ar, L_trans, R_trans);
			tree[j][k].L = R_trans.transpose();
			tree[j][k].R = L_trans.transpose();
			if(ComputedRank > 0) {
				for (int r = 0; r < row_bases.size(); r++) {
					tree[j][k].incoming_checkPoints.push_back(boxA_Nodes[col_bases[r]]);
				}
				for (int c = 0; c < col_bases.size(); c++) {
					tree[j][k].incoming_chargePoints.push_back(IL_Nodes[row_bases[c]]);
				}
			}
		}
	}
	...
	void getNodes_incoming_box(int j, int k, int& ComputedRank) {
		...
		if(ComputedRank > 0) {
			for (int r = 0; r < row_bases.size(); r++) {
				tree[j][k].incoming_checkPoints_local.push_back(boxA_Nodes[row_bases[r]]);
			}
			for (int c = 0; c < col_bases.size(); c++) {
				tree[j][k].incoming_chargePoints_local.push_back(IL_Nodes[col_bases[c]]);
			}
	...
	void getNodes_incoming_level_topDown(int j) {
		int ComputedRank;
		for (int k=0; k<nBoxesPerLevel[j]; ++k) {
			getNodes_incoming_box_topDown(j, k, ComputedRank);
		}
	}
