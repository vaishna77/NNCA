...
NNCA (New Nested Cross Approximation) is an :math:`\mathcal{O}(N)` complexity algorithm to construct :math:`\mathcal{H}^{2}` matrices. :math:`\texttt{NNCAlib}` library constructs a H2 matrix representation of a given matrix using NNCA. Further it demonstrates:

  #. the performance of the fast :math:`\mathcal{H}^{2}` matrix vector product using NNCA in 2D, 3D, and nD
